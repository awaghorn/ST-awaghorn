/**
 *
 *  Qubino Flush 1D Relay modified for maglock use
 *
 *  Written by Andrew Waghorn, based on DTH by Eric Maycock
 *
...
metadata {
    definition (name: "Qubino Flush 1D Relay AMW", namespace: "awaghorn", author: "Andrew Waghorn", vid:"generic-switch-power-energy") {
        capability "Actuator"
        capability "Switch"
        capability "Polling"
        capability "Refresh"
    ...
    tiles {
        multiAttributeTile(name:"switch", type: "generic", width: 6, height: 4, canChangeIcon: true) {
            tileAttribute ("device.switch", key: "PRIMARY_CONTROL") {
                attributeState "off", label:'Unlocked', action:"switch.on", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
                attributeState "on", label:'Locked', action:"switch.off", icon:"st.switches.switch.on", backgroundColor:"#00a0dc", nextState:"turningOff"
                attributeState "turningOff", label:'Unlocking', action:"switch.on", icon:"st.switches.switch.off", backgroundColor:"#ffffff", nextState:"turningOn"
                attributeState "turningOn", label:'Locking', action:"switch.off", icon:"st.switches.switch.on", backgroundColor:"#00a0dc", nextState:"turningOff"
            }
        }
...
def zwaveEvent(physicalgraph.zwave.commands.sensorbinaryv2.SensorBinaryReport cmd) {
    ...
    logging("$childDevice with setting ${settings['i2']} and value ${cmd.sensorValue}", 2)
...
def updated() {
    ...
    /*
    if (childDevices) {
      deleteChildDevice("${device.deviceNetworkId}-i2")
    }
    */
