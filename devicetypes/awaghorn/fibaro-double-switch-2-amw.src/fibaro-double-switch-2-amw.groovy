 ...
 *
 *  Derived from DTH developed by Eric Maycock (c) 2016
 *
...
metadata {
definition (name: "Fibaro Double Switch 2 AMW", namespace: "awaghorn", author: "Andrew Waghorn") {
    capability "Sensor"
    capability "Actuator"
    capability "Switch"
    capability "Polling"
...
def childRefresh(String dni) {
	...
	def cmds = []
    cmds << secure(encap(zwave.switchBinaryV1.switchBinaryGet(), channelNumber(dni)))
    cmds << secure(encap(zwave.meterV2.meterGet(scale: 0), channelNumber(dni)))
    cmds << secure(encap(zwave.meterV2.meterGet(scale: 2), channelNumber(dni)))
...
def childReset(String dni) {
	...
	def cmds = []
    cmds << secure(encap(zwave.meterV2.meterReset(), channelNumber(dni)))
    cmds << secure(encap(zwave.meterV2.meterGet(scale: 0), channelNumber(dni)))
    cmds << secure(encap(zwave.meterV2.meterGet(scale: 2), channelNumber(dni)))
    delayBetween(cmds,250)
}
