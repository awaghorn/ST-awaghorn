/*****************************************************************************************************************
 *  Copyright: Andrew Waghorn (awaghorn)
 *
 *  Name: Fibaro Dimmer 2
 *
 *  Author: Andrew Waghorn (awaghorn)
 *
 *  Date: 2019-02-08
 *
 *  Version: 1.00.2.02
 *
 *  Source: https://github.com/awaghorn/ST-awaghorn/tree/masterdevices/fibaro-dimmer-2
 *
 *  Author: Andrew Waghorn (awaghorn)
 *
 *  Description: An advanced SmartThings device handler for the Fibaro Dimmer 2 (FGD-212) Z-Wave Dimmer.
 *
 *	Based heavily on the Fibaro Dimmer 2 written by David Lomas (codersaur) available at:
 ...
 *****************************************************************************************************************/
metadata {
    definition (name: "Fibaro Dimmer 2", namespace: "awaghorn", author: "Andrew Waghorn") {
        capability "Actuator"
        capability "Switch"
        ...
        attribute "lastphysoff", "string"		// Last time that the device was physically switched off
		attribute "PhysLagSec", "number"		// 	Delay to apply
    ...
    preferences {
        ...
        section { // Configuration of delayed on response:
            input (
                type: "paragraph",
                element: "paragraph",
                title: "Physical Switch Lag:",
                description: "Managing repeated switching."
            )

            input (
                name: "configPhysLagOn",
                title: "Activate ignoring on commands for a period of time when device is switched off using S1?",
                type: "boolean",
                defaultValue: false, // iPhone users can uncomment these lines!
                required: true
            )
            ...
            input (
                name: "configPhysLagSec",
                title: "Delay applied to on commands when device switched off using S1.",
                type: "number",
...
def zwaveEvent(physicalgraph.zwave.commands.sceneactivationv1.SceneActivationSet cmd) {
    ...
    if (cmd.sceneId == 11) { //per docs S1 on to off sets scene 11
    	state.lastphysoff = now() //record last time switch was turned off
    	logger("Switch last off time updated","trace")
    }
...
def on() {
    ...
    logger("on(): configphyslagon: ${configphyslagon} ; lastphysoff: ${state.lastphysoff}","trace")
	
    if ( configPhysLagOn && now() < state.lastphysoff + state.PhysLagSec /* Delay active and within timeframe unless override */ ) {
		logger("on(): Dimmer on command skipped due to recent physical off switch activation.","info")
	} else {
...
def setLevel(level) {
	...
	logger("setlevel(): configphyslagon: ${configPhysLagOn} ; lastphysoff: ${state.lastphysoff}","trace")
  	if ( configPhysLagOn && now() < state.lastphysoff + state.PhysLagSec /* Delay active and within timeframe unless override */ ) {
    	logger("setlevel(): Dimmer set command skipped due to recent physical off switch activation.","info")
	} else {
...
def updated() {
    ...
    if (!state.updatedLastRanAt || now() >= state.updatedLastRanAt + 2000) {
        ...
        state.PhysLagSec 		  = 1000 * settings.configPhysLagSec.toInteger() //convert to milliseconds
